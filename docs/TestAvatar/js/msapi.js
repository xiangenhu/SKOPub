...
function playTTS(id, action,note) {
    ...
    var audioURL = animateBase + "?character=" + char.character + "&format=" + char.format + "&voice=" + char.voice + "&action=" + encodeURIComponent(action) + "&state=" + char.state + "&audio=true"; 
    audio.src = audioURL;
    audio.play()
}
...
function execute(id, action,note) {
	...
	var char = chars[id];
	savedURL = animateBase + "?character=" + char.character + "&format=" + char.format + "&voice=" + char.voice + "&action=" + encodeURIComponent(action) + "&state=" + char.state;
...
function loadLRS(alrsL){
	if (alrsL=="alttaiCCNU"){
		...
		LRSURL=qs("lrs","https://target-dev.soartech.com:8001/data/xAPI/");
		...
		LRSURL=qs("lrs","https://lrs.adlnet.gov/xAPI/");
		LRSLogin=qs("lrslogin","SKOAdmin");
		LRSPassword=qs("lrspassword","password"); 
	}
}
